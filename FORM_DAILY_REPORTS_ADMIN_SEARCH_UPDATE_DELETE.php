...
//            project_array=value_array[1];
            ...
            if(allmindate=='01-01-1970' && odmindate=='01-01-1970')
            {
    ...
    $(document).on('change','#ASRC_UPD_DEL_rd_actveemp',function(){
        if($('#ASRC_UPD_DEL_rd_actveemp').attr('checked',true))
        {
            if(active_emp.length!=0){
                ...
                $('#ASRC_UPD_DEL_errmsg').text(err_msg[10]).show();
                $('#ASRC_UPD_DEL_lbl_loginid').hide();
                $('#ASRC_UPD_DEL_lb_loginid').val('SELECT').hide();
            }
        }
        $('#ASRC_UPD_DEL_tble_paint').hide();
        $('#ASRC_UPD_DEL_lbl_btwnranges').show();
        ...
        else
        {
            ...
            xmlhttp.onreadystatechange=function() {
                if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                    ...
                    $('#ASRC_UPD_DEL_tbl_htmltable').hide();
                    $("#ASRC_UPD_DEL_btn_search").attr("disabled", "disabled");
                    $("#ASRC_UPD_DEL_tble_reasonlbltxtarea,#ASRC_UPD_DEL_secndselectprojct,#ASRC_UPD_DEL_tble_enterthereport,#ASRC_UPD_DEL_mrg_projectlistbx,#ASRC_UPD_DEL_aftern_projectlistbx,#ASRC_UPD_DEL_btn_allsearch").html('')
                    $('#ASRC_UPD_DEL_lbl_session').hide();
                    $('#ASRC_UPD_DEL_lbl_reason').hide();
                    $('#ASRC_UPD_DEL_ta_reason').hide();
...
//                        }
                        if(permission==null)
                        {
                            ...
                            if(emp_report==null)
                            {
                                ...
                                ASRC_UPD_DEL_table_header+='<tr ><td><input type="radio" name="ASRC_UPD_DEL_rd_flxtbl" class="ASRC_UPD_DEL_class_radio" id='+id+'  value='+id+' ></td><td align="center" >'+emp_date+'</td><td style="max-width:1000px; !important;"> '+ure_after_mrg +'  -  '+'REASON:'+emp_reason+'</td><td style="width:150px">'+userstamp+'</td><td style="min-width:90px;" nowrap>'+timestamp+'</td></tr>';
                            }
                            else if(emp_reason==null)
                            {
                                ASRC_UPD_DEL_table_header+='<tr ><td><input type="radio" name="ASRC_UPD_DEL_rd_flxtbl" class="ASRC_UPD_DEL_class_radio" id='+id+'  value='+id+' ></td><td align="center" >'+emp_date+'</td><td style="max-width:1000px; !important;">'+emp_report+'</td><td style="width:150px">'+userstamp+'</td><td style="min-width:90px;" nowrap>'+timestamp+'</td></tr>';
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                ASRC_UPD_DEL_table_header+='<tr ><td><input type="radio" name="ASRC_UPD_DEL_rd_flxtbl" class="ASRC_UPD_DEL_class_radio" id='+id+'  value='+id+' ></td><td align="center" >'+emp_date+'</td><td style="max-width:1000px; !important;">'+emp_report+' <br>PERMISSION:'+permission+' hrs</td><td style="width:150px">'+userstamp+'</td><td style="min-width:90px;" nowrap>'+timestamp+'</td></tr>';
                            }
                            else
                            {
...
//            $('#ASRC_UPD_DEL_tble_bandwidth').html('');
            ...
            if($('#ASRC_UPD_DEL_lb_attendance').val()=='1')
            {
                ...
                $('#ASRC_UPD_DEL_tble_enterthereport,#ASRC_UPD_DEL_ta_reason').html('');
...
//        $('#ASRC_UPD_DEL_banerrmsg').hide();
    ...
    $('#ASRC_UPD_DEL_lb_ampm').change(function(){
        projectid_array='';
        $('#ASRC_UPD_DEL_tble_paint').hide();
        $('#ASRC_UPD_DEL_tble_reasonlbltxtarea,#ASRC_UPD_DEL_tble_enterthereport,#ASRC_UPD_DEL_tble_frstsel_projectlistbx').html('');
...
//            }
    ...
    function ASRC_paint(){
        $('<tr><td width="150"><label name="ASRC_lbl_paint" id="ASRC_lbl_paint" >DRAWING SURFACE</label></td><td><div id="wPaint" class="isResizable update_validate"></div><center id="wPaint-img"></center></td></tr>').appendTo($("#ASRC_UPD_DEL_tble_paint"));
    }
    //FORM VALIDATION
...
//        var ASRC_UPD_DEL_bndtxt = $("#ASRC_UPD_DEL_tb_band").val();
        ...
        if(err_flag!=1){
            ...
            {
                if(((ASRC_UPD_DEL_reasontxtarea.trim()!="")&&(ASRC_UPD_DEL_reportenter!='')&&( ASRC_UPD_DEL_projectselectlistbx>0) && ((ASRC_UPD_DEL_permission==true) || (ASRC_UPD_DEL_nopermission==true))))
                {
            ...
            {
                if(((ASRC_UPD_DEL_reportenter.trim()!="")&&( ASRC_UPD_DEL_projectselectlistbx>0) && ((ASRC_UPD_DEL_permission==true) || (ASRC_UPD_DEL_nopermission==true))))
                {
                    if(ASRC_UPD_DEL_permission==true)
                    {
                        if(ASRC_UPD_DEL_permissionlstbx!='SELECT')
    ...
    $(document).on('click','#ASRC_UPD_DEL_btn_submit',function(){
        ...
        var formElement = $('#ASRC_UPD_DEL_form_adminsearchupdate').serialize();
        var choice="ADMIN REPORT SEARCH UPDATE DELETE";
        // Send Request
        request = $.ajax({
            url: "DB_DAILY_REPORTS_ADMIN_SEARCH_UPDATE_DELETE.do",
            type: "POST",
            ...
            success: function(response){
                ...
                {
                    var foldererrmsg=err_msg[14].replace("[FID]",parnt_foldid);
                    $(document).doValidation({rule:'messagebox',prop:{msgtitle:"ADMIN SEARCH AND UPDATE",msgcontent:foldererrmsg,position:{top:150,left:500}}});
                    ASRC_UPD_DEL_clear()
                    flextablerange()
                    $("#ASRC_UPD_DEL_btn_del").hide();
                    $("#ASRC_UPD_DEL_btn_srch").hide();
    ...
    $(document).on('change','#option',function(){
        ...
        else if($('#option').val()=='ONDUTY REPORT SEARCH UPDATE')
        {
            ...
            $('#ASRC_UPD_DEL_tbl_entry').hide();
            $('#ASRC_UPD_DEL_tbl_htmltable').hide();
            if(odmindate=='01-01-1970')
            {
                $('#ASRC_UPD_DEL_oderrmsg').text(err_msg[10]).show();
            }
            else
            {
                $('#ASRC_UPD_DEL_oderrmsg').show();
...
//    var imagelink='https://docs.google.com/a/ssomens.com/uc?id=0Bzvv-O9jT9r_Zmd4WUttUXhFc2M&export=download';
//    var wp=$("#wPaint").wPaint('image',imagelink);
    ...
    <form   id="ASRC_UPD_DEL_form_adminsearchupdate" class="newcontent" >
        ...
        <table id="ASRC_UPD_DEL_tble_dailyuserentry" hidden>
            ...
            <table id="ASRC_UPD_DEL_tble_attendence">
                ...
                <tr>
                    <td nowrap><input type="radio" name="permission" id="ASRC_UPD_DEL_rd_nopermission" value="NOPERMISSION" class='permissn update_validate'  hidden >
                        <label name="ASRC_UPD_DEL_nopermission" id="ASRC_UPD_DEL_lbl_nopermission" hiddeen nowrap>NO PERMISSION<em>*</em></label></td>
                </tr>
                <tr>
            ...
            <table id="ASRC_UPD_DEL_tble_paint" width=820"></table>
