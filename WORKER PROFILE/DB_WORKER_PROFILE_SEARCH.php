...
if($_REQUEST['option']=='search_data')
{
    ...
    if($activeemp!=''){
        $select_data=mysqli_query($con,"SELECT ULD_ID,ULD_USERNAME,ULD_PASSWORD,RC_NAME,ULD_WORKER_NAME,ULD_WORKER_NO,ULD_DOC_FOLDER_ID,ULD_DOC_FILE_NAME,ULD_EMAIL_ID,ULD_ADDRESS,ULD_NRIC_NO,ULD_MOBILE_NUMBER FROM LMC_USER_LOGIN_DETAILS A,LMC_ROLE_CREATION B WHERE A.RC_ID=B.RC_ID AND A.ULD_ID='$activeemp' ");
        while($row=mysqli_fetch_array($select_data)){
            $wrp_name=$row["ULD_WORKER_NAME"];
            $wrp_no=$row["ULD_WORKER_NO"];
            $wrp_uname=$row["ULD_USERNAME"];
            ...
            $ure_values=array('folder_id'=>$wrp_folderid,'wrp_name' =>$wrp_name,'wrp_no' =>$wrp_no,'wrp_rcid' =>$wrp_rcid,'wrp_filename'=>$wrp_filename,'wrp_rowid'=>$wrp_rowid,'wrp_emailid'=>$wrp_emailid,'wrp_address'=>$wrp_address,'wrp_nricno'=>$wrp_nricno,'wrp_mobno'=>$wrp_mobno,'wrp_uname'=>$wrp_uname,'wrp_pswd'=>$wrp_pswd);
...
//STRING REPLACE FUNCTION
        ...
        $replace= array("[USERNAME]","PERSONAL DETAILS:","[WNAME]","[WNO]","[NRICNO]","[EMAILID]","[MOBNO]","[ADDRESS]");
        $str_replaced  = array($empname,'<b>'."PERSONAL DETAILS:".'</b>',$empname,$empnum,$nricno,$emailid,$contactno,$comment_msg_add);
